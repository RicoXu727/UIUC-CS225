...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* buildTree(vector<Point<Dim>>& copyPoints, unsigned left, unsigned right, int d);
    ...
    int quick_partition(vector<Point<Dim>>& list, int left, int right, int pivotIndex, int d);

    Point<Dim> quick_select(vector<Point<Dim>>& list, int left, int right, int k, int d);
    ...
    Point<Dim> findhelper(KDTreeNode * root, const Point<Dim>& query, int d) const;
  
    void destroy_(KDTreeNode * root);
    void copy_(KDTreeNode * root, KDTreeNode * other);
