                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (mergeQueue.size() == 0) {
        smalltmp = singleQueue.front();
        singleQueue.pop();
    } else if (singleQueue.size() == 0) {
        smalltmp = mergeQueue.front();
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned i = 0; i < frequencies.size(); i++) {
        TreeNode * buildtmp = new TreeNode(frequencies[i]);
        singleQueue.push(buildtmp);
    }

    while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode * firsttmp = removeSmallest(singleQueue, mergeQueue);
        TreeNode * secondtmp = removeSmallest(singleQueue, mergeQueue);
        TreeNode * node = new TreeNode(firsttmp->freq.getFrequency() + secondtmp->freq.getFrequency()); 
        node->left = firsttmp;
        node->right = secondtmp;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit()) {
        ...
        readtmp = new TreeNode(newf);
    } else {
        Frequency newf(0);
        readtmp = new TreeNode(newf);
        readtmp->left = readTree(bfile);
        readtmp->right = readTree(bfile);
    }
    return readtmp;
}
