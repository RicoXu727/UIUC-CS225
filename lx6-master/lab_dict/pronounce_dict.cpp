...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    for (auto & c1 : copy_word1) {
        c1 = toupper(c1);
    }
    for (auto & c2 : copy_word2) {
        c2 = toupper(c2);
    }

    auto lookup1 = dict.find(copy_word1);
    auto lookup2 = dict.find(copy_word2);

    if (lookup1 == dict.end() || lookup2 == dict.end()) {
        return false;
    }
    vector<string> vector1 = (*lookup1).second;
    vector<string> vector2 = (*lookup2).second;
